...
uv_handle_t* Http2StreamGetHandle(UbiStreamBase*) {
  return nullptr;
}

uv_stream_t* Http2StreamGetLibuvStream(UbiStreamBase*) {
  return nullptr;
}

int Http2StreamWriteBufferDirect(UbiStreamBase* base,
                                 napi_value req_obj,
                                 napi_value payload,
                                 bool* async_out) {
  ...
  if (req_obj != nullptr) {
    UbiStreamReqActivate(base->env, req_obj, kUbiProviderWriteWrap, wrap->async_id);
  }
...
const UbiStreamBaseOps kHttp2StreamOps = {
...
void SetStreamWriteState(napi_env env, size_t bytes_written, bool async) {
  int32_t* stream_state = UbiGetStreamBaseState(env);
  if (stream_state == nullptr) return;
  stream_state[kUbiBytesWritten] = static_cast<int32_t>(std::min<size_t>(bytes_written, INT32_MAX));
  stream_state[kUbiLastWriteWasAsync] = async ? 1 : 0;
}
...
bool SetReadState(napi_env env, int32_t nread, int32_t offset = 0) {
  ...
  state[kUbiArrayBufferOffset] = offset;
  return true;
}
...
bool CallCallbackRef(napi_env env,
  ...
  return UbiAsyncWrapMakeCallback(
             env, async_id, recv, recv, callback, argc, argv, out, kUbiMakeCallbackNone) == napi_ok;
}
...
bool CallCallbackRefWithResource(napi_env env,
  ...
  napi_value ignored = nullptr;
  napi_value* out = result != nullptr ? result : &ignored;
  return UbiAsyncWrapMakeCallback(env,
                                  async_id,
...
bool InitPendingAsyncResource(napi_env env,
  ...
  *async_id = UbiAsyncWrapNextId(env);
  const int64_t trigger_async_id = UbiAsyncWrapExecutionAsyncId(env);
  UbiAsyncWrapEmitInitString(env, *async_id, type, trigger_async_id, resource);
  return true;
}
...
void CompletePendingSettings(napi_env env, PendingSettings* pending, bool ack) {
  ...
  (void)CallCallbackRefWithResource(env,
                                    ...
                                    2,
                                    argv);
  if (pending->async_id > 0) UbiAsyncWrapQueueDestroyId(env, pending->async_id);
...
void CompleteReqRef(napi_env env, napi_ref* ref, int status) {
  ...
  DeleteRefIfPresent(env, ref);
  if (req != nullptr) UbiStreamBaseInvokeReqOnComplete(env, req, status, nullptr, 0);
}
...
void CompleteStreamReq(Http2StreamWrap* stream, napi_ref* ref, int status, bool shutdown = false) {
  ...
  if (shutdown) {
    UbiStreamBaseEmitAfterShutdown(&stream->base, req, status);
  } else {
    UbiStreamBaseEmitAfterWrite(&stream->base, req, status);
  }
}
...
int WriteBufferToParent(Http2SessionWrap* session,
  ...
  if (session->parent_stream_base != nullptr) {
    return UbiStreamBaseWriteBufferDirect(session->parent_stream_base, req_obj, buffer, async_out);
  }
  ...
  if (async_out != nullptr && rc == 0) {
    int32_t* stream_state = UbiGetStreamBaseState(session->env);
    *async_out = stream_state != nullptr && stream_state[kUbiLastWriteWasAsync] != 0;
  }
  return rc;
}
...
int WriteChunksToParent(Http2SessionWrap* session, napi_value req_obj, bool* async_out) {
  ...
  if (session->parent_stream_base != nullptr) {
    return UbiStreamBaseWritevDirect(session->parent_stream_base, req_obj, chunks, async_out);
  }
  ...
  napi_value argv[3] = {req_obj, chunks, UbiStreamBaseMakeBool(session->env, true)};
  napi_value result = nullptr;
...
void EmitInboundEOF(Http2StreamWrap* stream) {
  ...
  if (UbiStreamBaseEmitEOF(&stream->base)) {
  ...
  (void)UbiAsyncWrapMakeCallback(
      stream->env, stream->async_id, self, self, callback, 1, argv, &ignored, kUbiMakeCallbackNone);
...
void NotifySessionDone(Http2SessionWrap* session, bool resume_parent_reading) {
  ...
  if (session->destroyed && session->parent_stream_base != nullptr) {
    (void)UbiStreamBaseRemoveListener(session->parent_stream_base, &session->parent_stream_listener);
...
napi_value DeferredDestroyStreamCallback(napi_env env, napi_callback_info info) {
  ...
  UbiStreamBaseOnClosed(&stream->base);
  return Undefined(env);
}
...
void CompleteParentWriteRequest(Http2SessionWrap* session, napi_value req_obj, int status) {
  ...
  if (session->parent_stream_base != nullptr) {
    UbiStreamBaseEmitAfterWrite(session->parent_stream_base, req_obj, status);
  } else {
    UbiStreamBaseInvokeReqOnComplete(session->env, req_obj, status, nullptr, 0);
  }
}
...
int FlushSessionOutput(Http2SessionWrap* session, napi_value req_obj) {
  ...
  napi_value actual_req = req_obj != nullptr ? req_obj : UbiCreateStreamReqObject(session->env);
...
InboundDelivery DeliverInboundChunk(Http2StreamWrap* stream, const uint8_t* data, size_t len) {
  ...
  if (UbiStreamBaseEmitReadBuffer(&stream->base, data, len)) {
...
napi_value SessionUpdateChunksSent(napi_env env, napi_callback_info info) {
  ...
  if (wrap != nullptr) {
    ...
    napi_set_named_property(env, self, "chunksSentSinceLastWrite", UbiStreamBaseMakeDouble(env, value));
  }
...
napi_value SessionConsume(napi_env env, napi_callback_info info) {
  ...
  wrap->parent_stream_base = UbiStreamBaseFromValue(env, argv[0]);
  ...
  if (wrap->parent_stream_base != nullptr) {
    ...
    (void)UbiStreamBasePushListener(wrap->parent_stream_base, &wrap->parent_stream_listener);
  }
...
napi_value StreamWriteBuffer(napi_env env, napi_callback_info info) {
  ...
  if (argc >= 1 && argv[0] != nullptr) {
    UbiStreamReqActivate(env, argv[0], kUbiProviderWriteWrap, wrap != nullptr ? wrap->async_id : -1);
  }
...
napi_value StreamSetOnRead(napi_env env, napi_callback_info info) {
  ...
  if (wrap != nullptr) {
    ...
    (void)UbiStreamBaseSetOnRead(&wrap->base, onread_value);
    DeleteRefIfPresent(env, &wrap->onread_ref);
    if (argc >= 1 && IsFunction(env, argv[0])) {
...
napi_value SessionCtor(napi_env env, napi_callback_info info) {
  ...
  UbiAsyncWrapEmitInitString(env, wrap->async_id, "HTTP2SESSION", UbiAsyncWrapExecutionAsyncId(env), self);
  ...
  (void)napi_set_named_property(env, self, "chunksSentSinceLastWrite", UbiStreamBaseMakeDouble(env, 0));
  return self;
}
...
napi_value StreamCtor(napi_env env, napi_callback_info info) {
  ...
  wrap->env = env;
  wrap->async_id = UbiAsyncWrapNextId(env);
  UbiStreamBaseInit(&wrap->base, env, &kHttp2StreamOps, kUbiProviderNone);
  if (argc >= 1) {
    int32_t id = 0;
  ...
  UbiAsyncWrapEmitInitString(env, wrap->async_id, "HTTP2STREAM", UbiAsyncWrapExecutionAsyncId(env), self);
  ...
  napi_value external = UbiStreamBaseGetExternal(&wrap->base);
  if (external != nullptr) {
