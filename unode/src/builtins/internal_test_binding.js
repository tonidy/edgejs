...
// Runtime internal/test/binding shim used by Node JS builtins.
const UV_ENOENT = -2;
const UV_EEXIST = -17;
const kHasBackingStore = new WeakSet();
...
// Runtime internal binding bridge used by Node JS builtins.
const UV_ENOENT = -2;
const UV_EEXIST = -17;
const kHasBackingStore = new WeakSet();
...
function internalBinding(name) {
  ...
  if (name === 'util') {
    return {
      ...
      getOwnNonIndexProperties(obj) {
        return Object.getOwnPropertyNames(obj).filter((name2) => {
          const index = Number(name2);
          return !Number.isInteger(index) || String(index) !== name2;
        });
      },
      isInsideNodeModules() {
        return false;
